...
class ResetUserDataView(APIView):
    """Delete all financial data for the authenticated user (keep account)."""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request):
        ...
        user = request.user
        inc_count  = Income.objects.filter(user=user).count()
        exp_count  = Expense.objects.filter(user=user).count()
        bud_count  = Budget.objects.filter(user=user).count()
        goal_count = SavingsGoal.objects.filter(user=user).count()
        ...
        return Response({
            'message': 'All financial data has been deleted.',
            'deleted': {
                'income': inc_count,
                'expenses': exp_count,
                'budgets': bud_count,
